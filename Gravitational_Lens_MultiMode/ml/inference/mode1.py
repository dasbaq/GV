...
def _optional_image(
    ...
    with h5py.File(input_path, "r") as h5:
        ...
        has_approx = "approx_outputs" in h5 and "S_approx" in h5["approx_outputs"]
        if not (has_image and has_approx):
            return image, False
        img_raw = _maybe_resize_image(h5["images/I_obs"][system_index], image_size)
        approx_raw = _maybe_resize_image(h5["approx_outputs/S_approx"][system_index], image_size)
    return np.stack([img_raw, img_raw - approx_raw], axis=0).astype(np.float32), True
...
def build_mode1_batch(
    *,
    ...
    lc[0, :n_valid] = F_joint[:n_valid]
    lc[1, :n_valid] = sigma_joint[:n_valid]
...
def run_mode1_correction(
    *,
    ...
    sigma_scaled = sigma_raw * float(mode1_sigma_scale)
