"""Phase 4 v0.3 infrastructure equivalence, retrain, and bootstrap eval.
...
DATA_NAME = "phase4_v0_3.h5"
UNFILTERED_DATA_NAME = "phase4_v0_3_eval_unfiltered.h5"
PATHS = build_round_paths(
    root=ROOT,
    data_name=DATA_NAME,
    round_name="phase4_v0_3_round",
    checkpoint_name="phase4_v0_3_imgres_best.pt",
    history_name="phase4_v0_3_imgres_long_history.json",
    eval_name="phase4_v0_3_imgres_h0_eval.json",
    infra_name="phase4_v0_3_infra_equivalence.json",
    scaler_name="target_scaler_phase4_v0_3.pkl",
    unfiltered_name=UNFILTERED_DATA_NAME,
    eval_unfiltered_name="phase4_v0_3_imgres_h0_eval_unfiltered.json",
)
DATA = PATHS.data
...
INFRA = PATHS.infra
RUNS = PATHS.runs
EQUIVALENCE = PATHS.work_root / "logs" / "phase4_v0_3_equivalence.json"
...
def load_floor_analysis() -> dict | None:
    path = ROOT / "data" / "logs" / "phase4_v0_3_floor_analysis.json"
    if not path.exists():
        return None
    with path.open() as f:
        return json.load(f)
...
def run_equivalence_phase(cfg: dict, target_device: torch.device, worker_candidate: int) -> dict:
    ...
    temp_scaler = create_target_scaler(DATA, RUNS / "target_scaler_phase4_v0_3_temp.pkl", cfg["seed"])
